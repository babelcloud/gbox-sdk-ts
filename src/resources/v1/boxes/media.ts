...
export class Media extends APIResource {
  ...
  deleteMedia(mediaName: string, params: MediaDeleteMediaParams, options?: RequestOptions): APIPromise<void> {
    ...
    return this._client.delete(path`/boxes/${boxId}/media/albums/${albumName}/media/${mediaName}`, {
      ...options,
      headers: buildHeaders([{ Accept: '*/*' }, options?.headers]),
    });
  }
  ...
  downloadMedia(
    ...
    const { boxId, albumName } = params;
    return this._client.get(path`/boxes/${boxId}/media/albums/${albumName}/media/${mediaName}/download`, {
      ...options,
   ...
   * @example
   * ```ts
   * const response = await client.v1.boxes.media.getMedia(
   *   'IMG_001.jpg',
   *   {
   *     boxId: 'c9bdc193-b54b-4ddb-a035-5ac0c598d32d',
   *     albumName: 'Pictures',
  ...
  getMedia(
    mediaName: string,
    params: MediaGetMediaParams,
    options?: RequestOptions,
  ): APIPromise<MediaGetMediaResponse> {
    const { boxId, albumName } = params;
    return this._client.get(path`/boxes/${boxId}/media/albums/${albumName}/media/${mediaName}`, options);
  }
   ...
   * @example
   * ```ts
   * const response = await client.v1.boxes.media.listMedia(
   *   'Pictures',
   *   { boxId: 'c9bdc193-b54b-4ddb-a035-5ac0c598d32d' },
   * );
   * ```
  ...
  listMedia(
    albumName: string,
    params: MediaListMediaParams,
    options?: RequestOptions,
  ): APIPromise<MediaListMediaResponse> {
    const { boxId } = params;
    return this._client.get(path`/boxes/${boxId}/media/albums/${albumName}/media`, options);
  }
 ...
 * List album media
 */
export interface MediaListMediaResponse {
  /**
...
export interface MediaGetMediaParams {
  /**
   * Box ID
   */
  boxId: string;
...
export interface MediaListMediaParams {
  /**
   * Box ID
   */
  boxId: string;
